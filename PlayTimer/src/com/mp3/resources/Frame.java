...
public final class Frame
{
	...
	private byte			syncmode = SoundStream.INITIAL_SYNC;
	private Crc16			crc;
	...
	void read_header(SoundStream stream, Crc16[] crcp) throws BitstreamException
	{
		...
		do
		{
			...
			if (syncmode == SoundStream.INITIAL_SYNC)
			{
				...
				if (((headerstring >>> 20) & 1) == 0) // SZD: MPEG2.5 detection
					if (h_version == MPEG2_LSF)
						...
						throw stream.newBitstreamException(SoundStream.UNKNOWN_ERROR);
			...
			if ((framesize >=0) && (framesizeloaded != framesize))
			{
				...
				throw stream.newBitstreamException(SoundStream.INVALIDFRAME);
			}
			if (stream.isSyncCurrentPosition(syncmode))
			{
				if (syncmode == SoundStream.INITIAL_SYNC)
				{
					syncmode = SoundStream.STRICT_SYNC;
