...
import com.mp3.resources.Decoder;
import com.mp3.resources.Frame;
import com.mp3.resources.SampleBuffer;
import com.mp3.resources.SoundStream;
...
public class MP3Player extends JPanel {
  ...
  private transient volatile JLabel text = Main.remainderlabel;
  private transient volatile long limit = 0;
  private transient volatile long current = System.currentTimeMillis();
  
  RemindTask reminder = null;
            ...
            while (true) {
              ...
              try {
                ...
                if (playingSource == null) {
                  ...
                  long audioFileLength = ((File) playlistObject).length();
                  int frameSize = format.getFrameSize();
                  float frameRate = format.getFrameRate();
                  float durationInSeconds = (audioFileLength / (frameRate));  
                  
                  AudioFile f = AudioFileIO.read(((File) playlistObject));
                  Tag tag = f.getTag();
                  
                  /*if (Reminder.timer != null) {
                	  Reminder.timer.cancel();
                	  Reminder.timer.purge();
                  }*/
                  
//                  Reminder reminder = new Reminder(f.getAudioHeader().getTrackLength(), Main.remainderlabel);
                  
                  if (reminder != null) {
                	  reminder.interrupt();
                  }
                  
                  limit = f.getAudioHeader().getTrackLength();

                  reminder = new RemindTask(limit, Main.remainderlabel);
            ...
            if (inputStream != null) {
              try {
                ...
                Main.statuslabel.setText("Ready");
              } catch (Exception e) {
  ...
  public void clearPlayList() {
	...
	extends Thread {
		...
		public RemindTask(long limit, JLabel text) {
