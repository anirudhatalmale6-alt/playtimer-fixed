...
public class Main {
	...
	static FileNameExtensionFilter mp3filter = new FileNameExtensionFilter("MP3 FILES", "mp3", "mp3");
	static JFileChooser csvChooser = new JFileChooser();
	static FileNameExtensionFilter csvfilter = new FileNameExtensionFilter("CSV FILES", "csv", "csv");
	...
	public static void main(String[] args) {
		...
		editor = new JSpinner.DateEditor(controller.getjSpinner2(), "h:mm:ss a");
		...
		controller.getjSpinner3()
				.setEditor(
						new JSpinner.DateEditor(controller.getjSpinner3(),
								"h:mm:ss a"));
		...
		controller.getjSpinner4()
				.setEditor(
						new JSpinner.DateEditor(controller.getjSpinner4(),
								"h:mm:ss a"));
		...
		controller.getjSpinner5()
				.setEditor(
						new JSpinner.DateEditor(controller.getjSpinner5(),
								"h:mm:ss a"));
		...
		controller.getjSpinner6()
				.setEditor(
						new JSpinner.DateEditor(controller.getjSpinner6(),
								"h:mm:ss a"));
		...
		controller.getjSpinner7()
				.setEditor(
						new JSpinner.DateEditor(controller.getjSpinner7(),
								"h:mm:ss a"));
		...
		if (serFile.exists()) {
			...
			// Grace period: 5 minutes - play missed schedules that were
			// supposed to trigger during system boot/startup
			long GRACE_PERIOD_MS = 5 * 60 * 1000;
			ScheduleTime mostRecentMissed = null;
			long mostRecentMissedAge = Long.MAX_VALUE;
			...
			for (Entry<String, List<ScheduleTime>> key : scheduleMap.entrySet()) {
				...
				for (ScheduleTime time : list) {
					...
					if (val >= 0) {
						// Future schedule - set timer normally
						reminderList.put(
								time.getStartDate().toString(),
								new PlayTimeScheduler(time.getStartDate().toString(), val, time
										.getMusicToBePlayed()));
					} else if (val >= -GRACE_PERIOD_MS) {
						// Recently missed (within 5 min) - track the most recent one
						long age = -val;
						if (age < mostRecentMissedAge) {
							mostRecentMissedAge = age;
							mostRecentMissed = time;
						}
					}
				}
			}

			// Play the most recently missed schedule immediately
			if (mostRecentMissed != null) {
				LOG.log.info("Playing missed schedule from startup: "
						+ mostRecentMissed.getStartDate());
				reminderList.put(
						mostRecentMissed.getStartDate().toString(),
						new PlayTimeScheduler(mostRecentMissed.getStartDate().toString(),
								0, mostRecentMissed.getMusicToBePlayed()));
			}
		}
		...
		load.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					csvschedule = DateConversionUtil.csvToSchedule(selectedFile);
					
					/*for (String key : csvschedule.keySet()) {
						
						System.out.println(key); 
						
						for (ScheduleTime time : csvschedule.get(key)) {
							System.out.println(time.getStartDate() + "  " + time.getMusicToBePlayed());
						}
					}*/
					...
					LOG.log.info("Loading new schedules from CSV");
					...
					LOG.log.error("Exception Occured while reading the csv file");
					JOptionPane.showMessageDialog(frame, ioe.getMessage());
				} catch (ParseException pe) {
					LOG.log.error("Unable to parse the csv file");
		...
		centerPanel.add(Box.createVerticalGlue()); // to place the panel in center when maximized
			...
			if ("Browse1".equalsIgnoreCase(e.getActionCommand())) {
				...
				if (scheduleMap.containsKey(key)) {
					...
					if (list.size() > 0
						...
						reminderList.get(list.get(0).getStartDate().toString())
								.cancel();
					}
					...
					Date newDate = getScheduleTime(controller
							.getjCalendarPanel1().getDate(), date.getHours(),
				...
				if (scheduleMap.containsKey(key)) {
					...
					if (list.size() > 1
							...
							&& reminderList.containsKey(list.get(1)
									.getStartDate().toString())) {
						reminderList.get(list.get(1).getStartDate().toString())
								.cancel();
					}
				...
				if (scheduleMap.containsKey(key)) {
					...
					if (list.size() > 2
							...
							&& reminderList.containsKey(list.get(2)
									.getStartDate().toString())) {
						reminderList.get(list.get(2).getStartDate().toString())
								.cancel();
					}
				...
				if (scheduleMap.containsKey(key)) {
					...
					if (list.size() > 3
							...
							&& reminderList.containsKey(list.get(3)
									.getStartDate().toString())) {
						reminderList.get(list.get(3).getStartDate().toString())
								.cancel();
					}
				...
				if (scheduleMap.containsKey(key)) {
					...
					if (list.size() > 4
							...
							&& reminderList.containsKey(list.get(4)
									.getStartDate().toString())) {
						reminderList.get(list.get(4).getStartDate().toString())
								.cancel();
					}
				...
				if (scheduleMap.containsKey(key)) {
					...
					if (list.size() > 5
							...
							&& reminderList.containsKey(list.get(5)
									.getStartDate().toString())) {
						reminderList.get(list.get(5).getStartDate().toString())
								.cancel();
					}
				...
				if (scheduleMap.containsKey(key)) {
					...
					if (list.size() > 6
							...
							&& reminderList.containsKey(list.get(6)
									.getStartDate().toString())) {
						reminderList.get(list.get(6).getStartDate().toString())
								.cancel();
					}
	...
	public void toggleComponents(boolean value) {
		...
		controller.getjSpinner6().setEnabled(value);
		controller.getjSpinner7().setEnabled(value);
		...
		controller.getjTextField6().setEnabled(value);
		controller.getjTextField7().setEnabled(value);
		...
		controller.getjButton6().setEnabled(value);
		controller.getjButton7().setEnabled(value);
	...
	public static void cancelReminders() {
		...
		LOG.log.info("Cancelling existing schedules started");
		for (String key : reminderList.keySet()) {
			PlayTimeScheduler sch = reminderList.get(key);
			LOG.log.info("Cancel Schedule " + sch.getStartDate());
			sch.cancel();
		}
		LOG.log.info("Cancelling existing schedules done");
	}
